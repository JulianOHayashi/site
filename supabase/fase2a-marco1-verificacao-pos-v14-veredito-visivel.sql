-- ============================================================================
-- BDFlow Site — FASE 2A · MARCO 1 · VERIFICAÇÃO PÓS-APLICAÇÃO — V14 (VEREDITO VISÍVEL, SOMENTE LEITURA)
-- (SOMENTE LEITURA)
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- VEREDITO
  -- ====================================================================
  -- VEREDITO VISIVEL: exibido por EXCECAO P0001 intencional. Continua SOMENTE
  -- LEITURA — nenhuma escritura ocorreu; o rollback da excecao e inofensivo.
  raise notice '-----------------------------------------------------------';
  raise notice 'Checagens aprovadas: %', v_ok;
  if array_length(v_falhas, 1) is null then
    raise exception using
      errcode = 'P0001',
      message = 'POST_FASE2A_MARCO1_OK',
      detail  = format('Checagens aprovadas: %s. Falhas: 0. Nenhuma divergencia.', v_ok);
  else
    raise exception using
      errcode = 'P0001',
      message = 'POST_FASE2A_MARCO1_COM_FALHAS',
      detail  = format('Checagens aprovadas: %s. Falhas: %s.%s%s',
                       v_ok, array_length(v_falhas, 1), chr(10),
                       array_to_string(v_falhas, chr(10)));
  end if;
end
$pos$;
