 ...
 *
 * Pacote P0 (correção): esta página NÃO consulta mais
 * `public.site_partner_members` — a auditoria de 2026-08-03 confirmou que
 * essa tabela NÃO existe no banco remoto. A versão anterior consultava essa
 * tabela para decidir se mostrava o CTA de cadastro; como a consulta falha
 * silenciosamente (o erro não era checado), o estado ficava indeterminado.
 *
 * Esta versão não tenta adivinhar se a conta já tem empresa vinculada — não
 * há hoje um jeito confiável de checar isso (a tabela de vínculo não
 * existe). Por isso mostramos diretamente, para qualquer conta autenticada,
 * o estado provisório "cadastro em preparação", sem inventar dado nem
 * simular vínculo. Isso volta a mudar quando a Fase 2A tiver o schema
 * definitivo do cadastro empresarial.
 *
 * Não recria site_partner_members. Não faz nenhuma consulta a tabela
 * inexistente.
 */
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 pb-24 pt-10">
        ...
        {/* Empresa parceira: estado provisório, sem depender de tabela inexistente */}
        <section className="mt-5 rounded-3xl border-2 border-dashed border-ciano/40 bg-ciano/5 p-8 text-center">
          <p className="text-2xl">🏢</p>
          <h2 className="mt-2 text-2xl font-bold">
            Cadastro de empresa em preparação
          </h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-tinta/70">
            O novo cadastro de empresas parceiras ainda está sendo preparado.
            A análise será feita pela BDFlow assim que estiver disponível.
          </p>
          <Link to="/parceiros/cadastro" className="btn-primary mt-5 inline-block">
            Ver preparação do cadastro
          </Link>
        </section>
